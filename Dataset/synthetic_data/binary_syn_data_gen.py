import numpy as np
from synthetic_data.test_utils import clingo_tensor_rule_check, tensor_rule_to_strings
...
def generate_data(NUM_NULLARY, NUM_CONJUNCTS, GEN_SIZE, RNG_SEED=0) -> str:
    ...
    target = np.concatenate(
        [np.ones(len(pos_nullary)), np.zeros(len(neg_nullary))], 0
    )
    data = {
        "nullary": nullary,
        "target": target,
